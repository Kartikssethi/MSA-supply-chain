...
def create_shipment(db: Session, origin: str, destination: str,  name:str, user_id: str,
    ...
    shipment = Shipment(origin=origin, destination=destination, name = name, driver_id = None, origin_lat = origin_lat, origin_long = origin_long, destination_lat = destination_lat,
    destination_long = destination_long, user_id=user_id)
    db.add(shipment)
    db.commit()
    db.refresh(shipment)
    return shipment
